...
if isTestPhase
    ...
    fprintf(resultFile, 'sigma,lambda,omega,objectiveScore,accuracy,trainingTime\n');
end

fprintf('(lambda, omega): (%f, %f)\n', lambda, omega);
...
%                     disp('Solve cvx A');
                    if i == sourceDomain
                        cvx_begin quiet
                            ...
                            minimize(norm(YMatrix{i}-U{i}*tmpA*sumFi*E'*V{i}',  'fro') + omega*norm((U{i}*tmpA*sumFi*E'*V{i}')'*L, 'fro'));
                        cvx_end
                    elseif i == targetDomain
                        cvx_begin quiet
                            variable tmpA(size(A));
                            minimize(norm((YMatrix{i}-U{i}*tmpA*sumFi*E'*V{i}').*W, 'fro') + omega*norm((U{i}*tmpA*sumFi*E'*V{i}')'*L, 'fro'));
                        cvx_end
                    end
                    % Assign cvx result
                    A = tmpA;
...
%                     disp('Solve cvx E');
                    if i == sourceDomain
                        cvx_begin quiet
                            ...
                            minimize(norm(YMatrix{i}-U{i}*A*sumFi*tmpE'*V{i}', 'fro') +  omega*norm((U{i}*A*sumFi*tmpE'*V{i}')'*L, 'fro'));
                        cvx_end
                    elseif i == targetDomain
                        cvx_begin quiet
                            variable tmpE(size(E));
                            minimize(norm((YMatrix{i}-U{i}*A*sumFi*tmpE'*V{i}').*W, 'fro') + omega*norm((U{i}*A*sumFi*tmpE'*V{i}')'*L, 'fro'));
                        cvx_end
                    end
                    % Assign cvx result
                    E = tmpE;
...
%                 end
            ...
            for i = 1:numDom
                ...
                objectiveScore = normEmp + smoothU + smoothY;
...
if isTestPhase
    for numResult = 1:randomTryTime
        fprintf(resultFile, '%f,%f,%f,%f,%f,%f\n', sigma, lambda, omega, resultCellArray{numResult}{1}, resultCellArray{numResult}{2}, resultCellArray{numResult}{3});
    end
