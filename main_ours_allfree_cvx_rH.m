...
%                     disp('Solve cvx A');
                    if i == sourceDomain
                        cvx_begin quiet
                            ...
                            minimize(norm(YMatrix{i}-U{i}*tmpA*sumFi*E'*V{i}',  'fro') + delta*norm(tmpA*sumFi*E', 'fro'));
                        cvx_end
                    elseif i == targetDomain
                        cvx_begin quiet
                            variable tmpA(size(A));
                            minimize(norm((YMatrix{i}-U{i}*tmpA*sumFi*E'*V{i}').*W, 'fro') + delta*norm(tmpA*sumFi*E', 'fro'));
                        cvx_end
                    end
                    % Assign cvx result
                    A = tmpA;
...
%                     disp('Solve cvx E');
                    if i == sourceDomain
                        cvx_begin quiet
                            ...
                            minimize(norm(YMatrix{i}-U{i}*A*sumFi*tmpE'*V{i}', 'fro') + delta*norm(A*sumFi*tmpE', 'fro'));
                        cvx_end
                    elseif i == targetDomain
                        cvx_begin quiet
                            variable tmpE(size(E));
                            minimize(norm((YMatrix{i}-U{i}*A*sumFi*tmpE'*V{i}').*W, 'fro') + delta*norm(A*sumFi*tmpE', 'fro'));
                        cvx_end
                    end
                    % Assign cvx result
                    E = tmpE;
...
%                 end
            ...
            for i = 1:numDom
                ...
                objectiveScore = normEmp + smoothU + smoothH;
...
if isTestPhase
    for numResult = 1:randomTryTime
        fprintf(resultFile, '%f,%f,%f,%f,%f,%f\n', sigma, lambda, delta, resultCellArray{numResult}{1}, resultCellArray{numResult}{2}, resultCellArray{numResult}{3});
    end
