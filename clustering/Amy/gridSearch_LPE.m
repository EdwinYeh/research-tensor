function [userResult] = gridSearch_LPE(X, P, S, alphas, betas)
% X: d*n data matrix
...
numOfComb = length(alphas)*length(betas);
userResult = cell(numOfComb, 3);%[alpha, beta], resultF, resultObj
initIter = 2;
...
for a=alphas
    for b=betas
        ...
        resultF = cell(initIter,1);
        for t=1:initIter
            [resultF{t,1},~,~,~,resultObj(t,1)] = LPE(X, P, S, a, b, 0); % [F, C, i, objFinal]
        end
