...
for i=fromUser:toUser
    ...
    for seedSetCount=1:seedSetNum
        ...
        % CCSKL
        reductionRate = 1;
        dimReduction = floor(k*reductionRate);
        if(dimReduction < 1) 
            dimReduction = 1;
        end
        ...
        for paramCount=1:diffParamNum
            ...
            for initIdx=1:initIterNum
                try
                newF = CCSKL(real(userDataMat{i}{paramCount,2})', k, S2, [], dimReduction); %return n*k
    ...
    save([outputPath algoResultMat num2str(fromUser) '-' num2str(toUser) '_CCSKL_m' num2str(reductionRate) '_recall_precision.mat'], 'recallsOfBestFScore','precisionsOfBestFScore', 'recall','filterPrecision','FScore','bestFScore','bestParameter','clusteringResult');
end
