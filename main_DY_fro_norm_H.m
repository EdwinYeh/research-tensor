...
%                     fprintf('Random:%d, Fold:%d, Iter:%d, Objective:%g\n', t, fold, iter, newObjectiveScore);
                    ...
                    for domId = 1:numDom
                        ...
                        [rA, cA] = size(A);
                        onesA = ones(rA, cA);
                        A = A.*sqrt((U{domId}'*YMatrix{domId}*V{domId}*E*sumFi+alpha*(onesA))./(delta*(A*sumFi*(E'*E)*sumFi'/norm(A*sumFi*E','fro'))+U{domId}'*U{domId}*A*sumFi*E'*V{domId}'*V{domId}*E*sumFi));
                        A(isnan(A)) = 0;
                        A(~isfinite(A)) = 0;
                        
                        [rE ,cE] = size(E);
                        onesE = ones(rE, cE);
                        E = E.*sqrt((V{domId}'*YMatrix{domId}'*U{domId}*A*sumFi + beta*(onesE))./(delta*(E*sumFi'*(A'*A)*sumFi/norm(A*sumFi*E','fro'))+V{domId}'*V{domId}*E*sumFi*A'*U{domId}'*U{domId}*A*sumFi));
                        E(isnan(E)) = 0;
                        E(~isfinite(E)) = 0;
        ...
        fprintf(resultFile, '%f,%g,%g,%g,%g,%f\n', bestAccuracy, sigma, sigma2, lambda, delta, bestTrainAndPredictTime);
        % matlabpool close;
    end
end
fclose(resultFile);
