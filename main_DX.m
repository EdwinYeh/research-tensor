...
for tuneGama = 0:0
    ...
    for tuneLambda = 0:0
        ...
        for t = 1: randomTryTime
            ...
            diff = Inf;
            convergeTimer = tic;
            while (diff >= 0.000001  && iter < maxIter)%(abs(oldObjectiveScore - newObjectiveScore) >= 0.1 && iter < maxIter)
                iter = iter + 1;
                ...
                for dom = 1:numDom
                    ...
                    V{dom}(isnan(V{dom})) = 0;
                    V{dom}(~isfinite(V{dom})) = 0;
                    tmpObjectiveScore = showObjectiveDX(U, V, X, Lu, Lv, CP1, CP2, CP3, CP4, lambda, gama);
                    if tmpObjectiveScore > oldObjectiveScore
                        fprintf('Objective increase in V(%f -> %f)\n', oldObjectiveScore, tmpObjectiveScore);
                    end
...
%                     %col normalize
                    ...
                    tmpObjectiveScore = showObjectiveDX(U, V, X, Lu, Lv, CP1, CP2, CP3, CP4, lambda, gama);
                    if tmpObjectiveScore > oldObjectiveScore
                        fprintf('Objective increase in U(%f -> %f)\n', oldObjectiveScore, tmpObjectiveScore);
                    end
                    %update fi
...
%                     disp('update A...');
                    ...
                    tmpObjectiveScore = showObjectiveDX(U, V, X, Lu, Lv, CP1, CP2, CP3, CP4, lambda, gama);
                    if tmpObjectiveScore > oldObjectiveScore
                        fprintf('Objective increase in A(%f -> %f)\n', oldObjectiveScore, tmpObjectiveScore);
                    end
                    ...
                    tmpObjectiveScore = showObjectiveDX(U, V, X, Lu, Lv, CP1, CP2, CP3, CP4, lambda, gama);
                    if tmpObjectiveScore > oldObjectiveScore
                        fprintf('Objective increase in E(%f -> %f)\n', oldObjectiveScore, tmpObjectiveScore);
                    end
