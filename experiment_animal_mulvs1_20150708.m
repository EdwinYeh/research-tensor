clear;clc;
disp('Test correctness of multiple vs 1, 5 instances and 10 features are used.');
...
numDom = 4;
targetDomain = 4;
domainNameList = {'source11.csv', 'source13.csv', 'source14.csv', 'target10.csv'};
createSparseTensor_multiple(domainNameList, numDom, 2);
trueLabel = cell(1, numDom);

for i = 1:numDom
    domainName = domainNameList{i};
    fprintf('Load domain:%s\n', domainName);
    label = load([prefix, domainName(1:length(domainName)-4), '_label.csv']);
    trueLabel{i} = label;
end

numSourceList = [3914 3907 3783 3954 3830 3823 1237 1016 897 5000 5000 5000 5000 5000 5000 5000];
numTargetList = [3926 3910 3336 3961 3387 3371 1207 1043 897 5000 5000 5000 5000 5000 5000 5000];
...
numInstance = [5000 5000 5000 5000 5000 5000 5000];
numFeature = [2688 2000 252 2000 2000 2000 10940];
numSampleInstance = [5 5 5 5 5 5 5];
numSampleFeature = [10 10 10 10 10 10 10];
numInstanceCluster = [41 41 41 11];
numFeatureCluster = [5 5 5 5];
...
resultFile = fopen('result_mul.txt', 'w');
...
for i = 1: numDom
    ...
    if isSampleInstance == true
        ...
        sampleInstance = X{i}(sampleInstanceIndex, :);
        numInstance(i) = numSampleInstance(i);
        Labels{i} = trueLabel{i}(sampleInstanceIndex, :);
        Labels{i} = trueLabel{i}(sampleInstanceIndex, :);
    end
    if isSampleFeature == true
        denseFeatures = findDenseFeature(sampleInstance, numSampleFeature(i));
        X{i} = sampleInstance(:, denseFeatures);
...
fprintf(resultFile, 'Multiple v.s. one');
