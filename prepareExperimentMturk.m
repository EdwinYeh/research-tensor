function [X, Y, XW, Su, Du, SeedCluster, PerceptionSeedFilter, SeedSet] = prepareExperimentMturk(expTitle, userIdList, sigmaInstsnce)
% Input:
...
for domId = 1:numDom;
    ...
    try
        ...
        [SeedSet{domId}, SeedCluster{domId}, PerceptionSeedFilter{domId}] = generateSeed(InstanceCluster, numPerception, 2);
    end
end
save(sprintf('SeedData_%s.mat', expTitle), 'SeedSet', 'SeedCluster', 'PerceptionSeedFilter');
end

function [SeedSet, ClusterSeed, PerceptionSeedFilter] = generateSeed(InstanceCluster, numPerception, seedCount)
[numInstance, numCluster] = size(InstanceCluster);
ClusterSeed = zeros(numInstance, numCluster);
...
for clusterId = 1: numCluster
    ...
    for seedId = 1: seedCount
        ClusterSeed(seed(seedId), clusterId) = 1;
